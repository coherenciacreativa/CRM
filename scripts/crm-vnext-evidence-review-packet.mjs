...
const DEFAULT_API_URL = 'http://localhost:3000/api/crm-vnext/evidence-review-packet';

const usage = `Usage:
  node scripts/crm-vnext-evidence-review-packet.mjs --text <text> [options]
  node scripts/crm-vnext-evidence-review-packet.mjs --text-file <path> [options]

Options:
  --api-url <url>          Evidence review packet API URL. Defaults to ${DEFAULT_API_URL}
  --text <text>            CRM facts/report text to review
  ...
  --decision-ledger-path <path>
                           Local evidence-review decisions JSONL ledger to suppress already-resolved questions
  --fail-on-open-review    Exit non-zero when review questions are present
  --help                   Show this help

This command is read-only. It turns preview evidence into review questions. It never mutates cards, never writes Fact Store, never sends outbound messages, and never calls live Gmail, Drive, or MailerLite APIs.`;
...
const runPacket = async (options) => {
  ...
  if (!text) throw new Error('evidence_review_packet_text_required');
  ...
  if (!response.ok) throw new Error(`evidence_review_packet_api_failed:${response.status}`);
  const payload = await response.json();
  if (!payload.ok) throw new Error(`evidence_review_packet_api_error:${payload.error ?? 'unknown'}`);
  return payload;
};
...
const compactReviewItem = (item) => ({
  ...
  candidateUpdates: item.candidateUpdates,
  ambiguousEmailCandidates: item.ambiguousEmailCandidates,
  possibleRelatedPeople: item.possibleRelatedPeople,
  decisionQuestions: item.decisionQuestions.map((question) => ({
  ...
  recommendedOperatorAction: item.recommendedOperatorAction,
  noWriteProof: item.noWriteProof,
});
...
const compactPacket = (packet, source) => ({
  ...
  reviewItems: packet.reviewItems.map(compactReviewItem),
...
const main = async () => {
  ...
  if (options.failOnOpenReview && payload.packet.summary.emailOwnershipQuestions > 0) {
    process.exitCode = 2;
  }
};

main().catch((error) => {
  console.error(`crm-vnext evidence-review-packet failed: ${error.message}`);
  process.exitCode = 1;
});
