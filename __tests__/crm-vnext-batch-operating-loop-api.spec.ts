...
import handler from "../pages/api/crm-vnext/batch-operating-loop.js";
...
const makeFixture = async () => {
  const dir = await mkdtemp(join(tmpdir(), "crm-vnext-batch-operating-loop-api-"));
  dirs.push(dir);
...
describe("/api/crm-vnext/batch-operating-loop", () => {
  test("serves one read-only operating loop without live sources or path leaks", async () => {
    ...
    const payload = res.body as {
      source: {
        cardStore: { existingStoreLoaded: boolean; localPathsRedacted: boolean };
        liveSources: {
          gmailLiveApiCalled: boolean;
          mailerLiteLiveApiCalled: boolean;
          googleDriveLiveApiCalled: boolean;
          contactsLiveApiCalled: boolean;
        };
      };
      loop: {
        summary: { items: number; evidenceQuestionQueueItems: number; operationsExecuted: number };
        evidenceQuestionQueue: Array<{ candidateEmail: string }>;
        safety: { batchLoopOnly: boolean; cardMutationProhibited: boolean };
      };
    };
    expect(payload.source.cardStore).toMatchObject({
      existingStoreLoaded: true,
    ...
    expect(payload.loop.summary).toMatchObject({
      items: 1,
      evidenceQuestionQueueItems: 1,
      operationsExecuted: 0,
    });
    expect(payload.loop.evidenceQuestionQueue[0].candidateEmail).toBe("mayaariana@hotmail.com");
  ...
  test("rejects missing text and non-POST methods", async () => {
    ...
    expect(missing.body).toEqual({ ok: false, error: "batch_operating_loop_text_required" });
