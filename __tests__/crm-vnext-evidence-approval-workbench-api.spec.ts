...
import handler from "../pages/api/crm-vnext/evidence-approval-workbench.js";
...
const makeFixture = async () => {
  const dir = await mkdtemp(join(tmpdir(), "crm-vnext-evidence-approval-workbench-api-"));
  dirs.push(dir);
...
describe("/api/crm-vnext/evidence-approval-workbench", () => {
  test("serves a read-only evidence approval queue without path leaks", async () => {
    ...
    const payload = res.body as {
      source: { liveSources: { gmailLiveApiCalled: boolean; mailerLiteLiveApiCalled: boolean; googleDriveLiveApiCalled: boolean } };
      workbench: {
        summary: { queueItems: number; recommendedKeepUnassigned: number; operationsExecuted: number };
        queueItems: Array<{ candidateEmail: string; recommendedDecisionCli: string }>;
        safety: { cardMutationProhibited: boolean; workbenchOnly: boolean };
      };
    };
    ...
    expect(payload.workbench.summary).toMatchObject({
      queueItems: 1,
      recommendedKeepUnassigned: 1,
      operationsExecuted: 0,
    });
    expect(payload.workbench.queueItems[0]).toMatchObject({
    ...
    expect(payload.workbench.safety).toMatchObject({
  ...
  test("rejects missing text and unsupported methods", async () => {
    process.env.NODE_ENV = "test";
    delete process.env.CRM_VNEXT_INSIGHTS_TOKEN;

    const missingText = mockRes();
    await handler({ method: "POST", query: {}, body: {}, headers: {} } as MockReq, missingText as never);
    expect(missingText.statusCode).toBe(400);
    expect(missingText.body).toEqual({ ok: false, error: "evidence_approval_workbench_text_required" });
