...
import handler from "../pages/api/crm-vnext/stitch-batch-review.js";
...
const makeFixture = async () => {
  const dir = await mkdtemp(join(tmpdir(), "crm-vnext-stitch-batch-review-api-"));
  dirs.push(dir);
...
describe("/api/crm-vnext/stitch-batch-review", () => {
  test("serves read-only batch reviews without live sources or path leaks", async () => {
    ...
    const payload = res.body as {
      ...
      batch: {
        summary: { items: number; openEvidenceQuestions: number; operationsExecuted: number };
        items: Array<{ recommendedAction: string; operationsExecuted: number }>;
        safety: { cardMutationProhibited: boolean; batchReviewOnly: boolean };
      };
    };
    ...
    expect(payload.batch.summary).toMatchObject({
      items: 1,
      openEvidenceQuestions: 1,
      operationsExecuted: 0,
    });
    expect(payload.batch.items[0]).toMatchObject({
      recommendedAction: "review_deferred_write",
      operationsExecuted: 0,
    });
    expect(payload.batch.safety).toMatchObject({
  ...
  test("rejects missing text and non-POST methods", async () => {
    ...
    expect(missing.body).toEqual({ ok: false, error: "stitch_batch_review_text_required" });
