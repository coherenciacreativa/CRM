...
import handler from "../pages/api/crm-vnext/gmail-evidence-helper.js";
...
const makeFixture = async () => {
  const dir = await mkdtemp(join(tmpdir(), "crm-vnext-gmail-evidence-api-"));
  dirs.push(dir);
...
describe("/api/crm-vnext/gmail-evidence-helper", () => {
  test("builds read-only Gmail evidence packets without calling live Gmail", async () => {
        ...
        gmailSearchResults: [
          {
            ...
            snippet: "Gladys Mayerli Garcia Ortegon has joined your Yoga Colombia meeting.",
          },
        ],
      },
      headers: {},
  ...
  test("rejects missing text and non-POST methods", async () => {
    ...
    expect(missing.body).toEqual({ ok: false, error: "gmail_evidence_text_required" });
