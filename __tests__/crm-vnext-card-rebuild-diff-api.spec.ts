...
import handler from "../pages/api/crm-vnext/card-rebuild-diff.js";
...
const makeFixture = async () => {
  const dir = await mkdtemp(join(tmpdir(), "crm-vnext-card-rebuild-diff-api-"));
  dirs.push(dir);
...
describe("/api/crm-vnext/card-rebuild-diff", () => {
  test("serves read-only card rebuild diffs without local path leaks", async () => {
    ...
    expect((res.body as { diff: { summary: { cardsWithDiffs: number } } }).diff.summary.cardsWithDiffs).toBe(1);
