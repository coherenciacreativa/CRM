...
describe("CRM vNext MailerLite mini-launch public send preflight decision packet", () => {
  ...
  test("explains the URL and audience decision without generating an approval phrase", () => {
    ...
    expect(report.executiveSummary.recommendedUrlDecisionId)
      .toBe("keep_existing_unlisted_noindex_preview_links_for_qa_and_micro_cohort_candidate");
    expect(report.executiveSummary.recommendedAudienceScopeId)
      .toBe("keep_null_audience_no_public_send");
    expect(report.executiveSummary.recommendedAudienceKnownActiveCount).toBe(0);
    expect(report.executiveSummary.recommendedDistributionPath)
      .toBe("qa_then_manual_micro_cohort_or_opt_in_testers_before_any_broad_send");
    ...
    expect(report.executiveSummary.audienceStrategyGateRequiredBeforeMassSend).toBe(true);
    ...
    expect(report.futureApprovalBoundary.phraseScope).toBe("pilot_distribution_strategy_decision_only_no_send");
    expect(report.futureApprovalBoundary.finalSendPhraseAvailable).toBe(false);
    expect(report.futureApprovalBoundary.whyFinalSendPhraseStillUnavailable).toContain("no mass subscriber send");
    expect(report.futureApprovalBoundary.stillClosedEvenAfterDecisionApproval).toContain("public_or_audience_send");
    expect(report.futureApprovalBoundary.stillClosedEvenAfterDecisionApproval)
      .toContain("mass_subscriber_or_existing_active_audience_send");
