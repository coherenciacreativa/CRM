...
const taxonomyRefreshResponseWorkspace = {
  ...
  executiveSummary: {
    ...
    canApplyCrmManifestPatchNow: false,
    openLiveMutationGateCount: 0,
    nextSafeAction: "collect_final_brand_and_crm_taxonomy_response_files_no_live_changes",
  },
  acceptedActors: [],
...
const taxonomyRefreshDecisionIntake = {
  ...
  executiveSummary: {
    ...
    canApplyCrmManifestPatchNow: false,
    openLiveMutationGateCount: 0,
    nextSafeAction: "collect_brand_and_crm_taxonomy_decisions_without_live_approval",
  },
  blockers: ["brand_decision_file_missing", "crm_decision_file_missing"],
  unsafeReasons: [],
};
...
describe("CRM vNext MailerLite Launch OS operator runbook", () => {
  ...
  test("summarizes current state with onboarding protected and live gates closed", () => {
    ...
    expect(state.miniLaunch.shopifyPreviewRouteDecisionStatus).toBe("shopify_preview_route_decision_ready_for_human_explanation_no_live_changes");
    expect(state.miniLaunch.shopifyPreviewRouteDecisionReady).toBe(true);
    expect(state.miniLaunch.shopifyPreviewRouteDecisionExplanationReady).toBe(true);
    expect(state.miniLaunch.shopifyPreviewRouteExactApprovalPhraseAvailable).toBe(false);
    expect(state.miniLaunch.shopifyPreviewRouteExactApprovalPhrasePrinted).toBe(false);
    ...
    expect(state.taxonomyRefreshResponseWorkspace).toMatchObject({
      status: "taxonomy_refresh_response_workspace_ready_awaiting_final_responses_no_live_changes",
      brandDecisionRowCount: 14,
      crmManifestPatchRowCount: 14,
      acceptedActorCount: 0,
    ...
    expect(state.taxonomyRefreshDecisionIntake).toMatchObject({
      status: "taxonomy_refresh_decision_intake_waiting_for_brand_crm_decisions_no_live_changes",
      brandDecisionRowsNeeded: 14,
      brandDecisionRowsPresent: 0,
      crmManifestPatchRowsNeeded: 14,
  ...
  test("builds report map from consulted source paths", () => {
    const reportMap = buildReportMap([
      ...
      {
        path: "/tmp/mailerlite_mini_launch_shopify_preview_route_decision_current_inteligencia_descansar_2026-05-31.json",
        present: true,
        chars: 2000,
        consultedFor: "mini-launch Shopify preview route decision boundary with no approval phrase or publish",
      },
      {
        path: "/tmp/mailerlite_mini_launch_shopify_preview_route_execution_receipt_current_inteligencia_descansar_2026-05-31.json",
        present: true,
        chars: 2000,
        consultedFor: "mini-launch Shopify preview route execution receipt with QA evidence and closed audience-send gate",
      },
      {
      ...
      {
        path: "/tmp/mailerlite_launch_os_taxonomy_refresh_response_workspace_2026-05-28.json",
        present: true,
        chars: 2000,
        consultedFor: "Launch OS Brand/CRM taxonomy response workspace with pending/final file separation",
      },
      {
        path: "/tmp/mailerlite_launch_os_taxonomy_refresh_decision_intake_2026-05-28.json",
        present: true,
        chars: 2000,
    ...
    expect(reportMap.miniLaunchShopifyPreviewRouteDecision).toBe("/tmp/mailerlite_mini_launch_shopify_preview_route_decision_current_inteligencia_descansar_2026-05-31.json");
    expect(reportMap.miniLaunchShopifyPreviewRouteExecutionReceipt).toBe("/tmp/mailerlite_mini_launch_shopify_preview_route_execution_receipt_current_inteligencia_descansar_2026-05-31.json");
  ...
  test("renders operator runbook with next moves and approval matrix", () => {
    ...
    expect(markdown).toContain("Taxonomy response workspace: taxonomy_refresh_response_workspace_ready_awaiting_final_responses_no_live_changes");
    expect(markdown).toContain("Taxonomy response pending actors: brand, crm");
