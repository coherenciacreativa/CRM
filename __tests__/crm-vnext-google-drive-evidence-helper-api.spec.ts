...
import handler from "../pages/api/crm-vnext/google-drive-evidence-helper.js";
...
const makeFixture = async () => {
  const dir = await mkdtemp(join(tmpdir(), "crm-vnext-google-drive-evidence-api-"));
  dirs.push(dir);
...
describe("/api/crm-vnext/google-drive-evidence-helper", () => {
  test("builds read-only Google Drive evidence packets without calling live Drive", async () => {
    ...
    const payload = res.body as {
      source: { googleDrive: { liveGoogleDriveCalledByApi: boolean; searchResultsSupplied: number } };
      helper: { summary: { evidenceSources: number }; evidenceSources: Array<{ sourceKind: string }> };
    };
    expect(payload.source.googleDrive).toEqual({
      liveGoogleDriveCalledByApi: false,
      searchResultsSupplied: 1,
    });
    expect(payload.helper.summary.evidenceSources).toBe(1);
    expect(payload.helper.evidenceSources[0].sourceKind).toBe("retreat_table");
  ...
  test("rejects missing text and non-POST methods", async () => {
    ...
    expect(missing.body).toEqual({ ok: false, error: "google_drive_evidence_text_required" });
