...
import handler from "../pages/api/crm-vnext/identity-stitching-research.js";
...
const makeFixture = async () => {
  const dir = await mkdtemp(join(tmpdir(), "crm-vnext-identity-stitching-api-"));
  dirs.push(dir);
...
describe("/api/crm-vnext/identity-stitching-research", () => {
  test("serves read-only stitching research without local path leaks", async () => {
    ...
    const research = (res.body as { research: { summary: { mailerReviewRecommendations: number }; clues: Array<{ recommendation: { action: string } }> } }).research;
    expect(research.summary.mailerReviewRecommendations).toBe(1);
    expect(research.clues[0].recommendation.action).toBe("review_mailer_candidate");
  ...
  test("rejects missing text and non-POST methods", async () => {
    ...
    expect(missing.body).toEqual({ ok: false, error: "identity_stitching_text_required" });
