...
} from '../../../lib/crm/crm-vnext-card-write-approval-packet';
import {
...
type ApiBody =
  | {
      ...
      packet: CrmCardWriteApprovalPacketReport;
    }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiBody>) {
  ...
  try {
    ...
    const packet = buildCrmVNextCardWriteApprovalPacket({
      text,
    ...
    console.error('crm-vnext card-write-approval-packet api error', error);
    return res.status(500).json({ ok: false, error: 'card_write_approval_packet_failed' });
  }
}
