...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiBody>) {
  ...
  try {
    return res.status(200).json({
      ok: true,
      brief: await buildCrmVNextEngagementDecisionBrief({
        ...sourceOptions,
    ...
    console.error('crm-vnext engagement-decision-brief api error', error);
    return res.status(500).json({ ok: false, error: 'engagement_decision_brief_failed' });
  }
}
