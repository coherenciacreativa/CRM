...
} from '../../../lib/crm/community-queue-briefs';
import {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiBody>) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      brief: buildCommunityQueueBrief(payload.cards, queueId, { limit }),
    });
  } catch (error) {
    console.error('crm-vnext community-queue-brief error', error);
    return res.status(500).json({ ok: false, error: 'community_queue_brief_failed' });
  }
}
