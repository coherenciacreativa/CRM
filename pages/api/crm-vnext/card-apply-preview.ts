...
} from '../../../lib/crm/crm-vnext-card-apply-preview';
import {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiBody>) {
  ...
  try {
    ...
    const preview = buildCrmVNextCardApplyPreview({
      text,
    ...
    console.error('crm-vnext card-apply-preview api error', error);
    return res.status(500).json({ ok: false, error: 'card_apply_preview_failed' });
  }
}
