...
} from '../../../lib/crm/crm-vnext-stitch-batch-review';
...
type ApiBody =
  | {
      ...
      batch: CrmStitchBatchReviewReport;
    }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiBody>) {
  ...
  try {
    ...
    if (!text) {
      return res.status(400).json({ ok: false, error: 'stitch_batch_review_text_required' });
    }
    ...
    const batch = buildCrmVNextStitchBatchReview({
      text,
    ...
    console.error('crm-vnext stitch-batch-review api error', error);
    return res.status(500).json({ ok: false, error: 'stitch_batch_review_failed' });
  }
}
