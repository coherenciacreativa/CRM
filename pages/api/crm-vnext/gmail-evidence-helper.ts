...
import {
  buildCrmVNextGmailEvidenceHelper,
  type CrmGmailEvidenceHelperReport,
} from '../../../lib/crm/crm-vnext-gmail-evidence-helper';
import {
...
type ApiBody =
  | {
      ...
      source: {
        ...
        gmail: {
          liveGmailCalledByApi: false;
          searchResultsSupplied: number;
        };
      };
      helper: CrmGmailEvidenceHelperReport;
    }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiBody>) {
  ...
  try {
    ...
    if (!text) {
      return res.status(400).json({ ok: false, error: 'gmail_evidence_text_required' });
    }
    ...
    const helper = buildCrmVNextGmailEvidenceHelper({
      text,
      ...
      mailerBridgeRows,
      gmailSearchResults: body.gmailSearchResults,
    });
    ...
    return res.status(200).json({
      ...
      source: {
        ...
        gmail: {
          liveGmailCalledByApi: false,
          searchResultsSupplied: suppliedResultsCount(body.gmailSearchResults),
        },
      },
      helper,
    });
  } catch (error) {
    console.error('crm-vnext gmail-evidence-helper api error', error);
    return res.status(500).json({ ok: false, error: 'gmail_evidence_helper_failed' });
  }
}
