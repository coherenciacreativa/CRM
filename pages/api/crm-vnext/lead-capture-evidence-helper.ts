...
import {
  buildCrmVNextLeadCaptureEvidenceHelper,
  type CrmLeadCaptureEvidenceHelperReport,
} from '../../../lib/crm/crm-vnext-lead-capture-evidence-helper';
import {
...
type ApiBody =
  | {
      ...
      helper: CrmLeadCaptureEvidenceHelperReport;
    }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiBody>) {
  ...
  try {
    ...
    if (!text) {
      return res.status(400).json({ ok: false, error: 'lead_capture_evidence_text_required' });
    }
    ...
    const helper = buildCrmVNextLeadCaptureEvidenceHelper({
      text,
      ...
      mailerBridgeRows,
      leadCaptureSearchResults: body.leadCaptureSearchResults,
    });
    ...
    return res.status(200).json({
      ...
      source: {
        ...
        leadCapture: {
          ...
          liveMailerLiteCalledByApi: false,
          liveWhatsAppCalledByApi: false,
          searchResultsSupplied: suppliedResultsCount(body.leadCaptureSearchResults),
        },
      },
      helper,
    });
  } catch (error) {
    console.error('crm-vnext lead-capture-evidence-helper api error', error);
    return res.status(500).json({ ok: false, error: 'lead_capture_evidence_helper_failed' });
  }
}
