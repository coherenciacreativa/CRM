...
import {
  buildCrmVNextGoogleDriveEvidenceHelper,
  type CrmGoogleDriveEvidenceHelperReport,
} from '../../../lib/crm/crm-vnext-google-drive-evidence-helper';
import {
...
type ApiBody =
  | {
      ...
      source: {
        ...
        googleDrive: {
          liveGoogleDriveCalledByApi: false;
          searchResultsSupplied: number;
        };
      };
      helper: CrmGoogleDriveEvidenceHelperReport;
    }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiBody>) {
  ...
  try {
    const body = typeof req.body === 'object' && req.body ? bodyFrom(req.body) : {};
    const text = cleanString(body.text);
    if (!text) {
      return res.status(400).json({ ok: false, error: 'google_drive_evidence_text_required' });
    }
    ...
    const helper = buildCrmVNextGoogleDriveEvidenceHelper({
      text,
      ...
      mailerBridgeRows,
      googleDriveSearchResults: body.googleDriveSearchResults,
    });
    ...
    return res.status(200).json({
      ...
      source: {
        ...
        googleDrive: {
          liveGoogleDriveCalledByApi: false,
          searchResultsSupplied: suppliedResultsCount(body.googleDriveSearchResults),
        },
      },
      helper,
    });
  } catch (error) {
    console.error('crm-vnext google-drive-evidence-helper api error', error);
    return res.status(500).json({ ok: false, error: 'google_drive_evidence_helper_failed' });
  }
}

const bodyFrom = (value: object): Record<string, unknown> => value as Record<string, unknown>;
