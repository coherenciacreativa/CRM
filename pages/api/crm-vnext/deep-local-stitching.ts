...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiBody>) {
  ...
  try {
    ...
    if (!text) {
      return res.status(400).json({ ok: false, error: 'deep_local_stitching_text_required' });
    }
    ...
    const stitching = buildCrmVNextDeepLocalStitching({
      text,
    ...
    console.error('crm-vnext deep-local-stitching api error', error);
    return res.status(500).json({ ok: false, error: 'deep_local_stitching_failed' });
  }
}
