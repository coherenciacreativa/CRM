...
import {
  buildCrmVNextMailerLiteEvidenceHelper,
  type CrmMailerLiteEvidenceHelperReport,
} from '../../../lib/crm/crm-vnext-mailerlite-evidence-helper';
import {
...
type ApiBody =
  | {
      ...
      helper: CrmMailerLiteEvidenceHelperReport;
    }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiBody>) {
  ...
  try {
    ...
    if (!text) {
      return res.status(400).json({ ok: false, error: 'mailerlite_evidence_text_required' });
    }
    ...
    const helper = buildCrmVNextMailerLiteEvidenceHelper({
      text,
      ...
      mailerBridgeRows,
      mailerLiteSearchResults: body.mailerLiteSearchResults,
    });
    ...
    return res.status(200).json({
      ...
      source: {
        ...
        mailerLite: {
          liveMailerLiteCalledByApi: false,
          searchResultsSupplied: suppliedResultsCount(body.mailerLiteSearchResults),
        },
      },
      helper,
    });
  } catch (error) {
    console.error('crm-vnext mailerlite-evidence-helper api error', error);
    return res.status(500).json({ ok: false, error: 'mailerlite_evidence_helper_failed' });
  }
}
