...
import {
  buildCrmVNextEvidenceApprovalApplication,
  type CrmEvidenceApprovalApplicationReport,
} from '../../../lib/crm/crm-vnext-evidence-approval-application';
import {
...
type ApiBody =
  | {
      ...
      application: CrmEvidenceApprovalApplicationReport;
    }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiBody>) {
  ...
  try {
    ...
    if (!text) return res.status(400).json({ ok: false, error: 'evidence_approval_application_text_required' });

    const decisions = cleanDecisions(body.decisions);
    if (!decisions.length) {
      return res.status(400).json({ ok: false, error: 'evidence_approval_application_decisions_required' });
    }
    const commit = cleanBool(body.commit);
    const approvedBy = cleanString(body.approvedBy);
    if (commit && !approvedBy) {
      return res.status(400).json({ ok: false, error: 'evidence_approval_application_approved_by_required' });
    }
    ...
    console.error('crm-vnext evidence-approval-application api error', error);
    return res.status(500).json({ ok: false, error: 'evidence_approval_application_failed' });
  }
}
