...
} from '../../../lib/crm/crm-vnext-evidence-review-packet';
import {
...
type ApiBody =
  | {
      ...
      packet: CrmEvidenceReviewPacketReport;
    }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiBody>) {
  ...
  try {
    ...
    if (!text) {
      return res.status(400).json({ ok: false, error: 'evidence_review_packet_text_required' });
    }
    ...
    const packet = buildCrmVNextEvidenceReviewPacket({
      text,
    ...
    console.error('crm-vnext evidence-review-packet api error', error);
    return res.status(500).json({ ok: false, error: 'evidence_review_packet_failed' });
  }
}
