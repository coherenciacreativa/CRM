...
- `status`: `assistant_reply_policy_boundary_designed_pending_integration`
...
- `latest_execution_note`: assistant reply policy boundary was designed as a
  ...
  lane work did not execute source actions.
- `next_approval_needed`: choose central integration review for the assistant
  reply policy boundary, controlled assistant reply draft preview, controlled
  one-reply send, CRM enrichment preview, MailerLite setup inventory, or pause.
