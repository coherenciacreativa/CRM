...
| MailerLite setup/config metadata | `npm run crm:vnext:mailerlite-setup-readonly-verification` / `scripts/crm-vnext-mailerlite-setup-readonly-verification.mjs` | fixture-tested; live-read pending approval | future live mode uses existing internal auth only; no credential output | redacted aggregate receipts; private refs only in private artifact if live-approved | none intended; live mode must use GET/read-only setup/config endpoints only | no for live until separately approved for this setup route | fixture tests only | redaction tests for IDs/emails/tokens/raw payloads | Dedicated onboarding setup/config readiness guard; fixture-first and live-blocked by default. | Central integration, then live read-only setup verification approval. |
