...
| MailerLite setup/config metadata | `npm run crm:vnext:mailerlite-setup-readonly-verification` / `scripts/crm-vnext-mailerlite-setup-readonly-verification.mjs` | live-read completed for setup/config metadata on 2026-07-06; mutation blocked | existing internal auth only after approval/path prechecks; no credential output | redacted aggregate receipts; private setup refs only in approved private artifact path | none intended; live mode enforces read-only setup/config route and no subscriber-row reads | no for mutation; yes for the approved live setup/config verification route only | completed live-readonly setup/config metadata on 2026-07-06 | v2 redaction-safe command; stdout/stderr redaction passed; redacted receipts passed; no subscriber rows; no raw IDs; setup/config metadata only | Dedicated onboarding setup/config readiness guard with GET-only setup allowlist and conservative mutation readiness. `mutation_readiness=blocked_field_mapping`. | Resolve setup drift / missing field mapping before any mutation review. |
| MailerLite packet-specific subscriber/status/group membership safety | `npm run crm:vnext:mailerlite-final-idempotency-suppression-check` / `scripts/crm-vnext-mailerlite-final-idempotency-suppression-check.mjs` | mocked-live-tested; real live-read pending approval | future live mode uses existing internal auth only after approval/path prechecks; no credential output | redacted aggregate receipts; private refs only in approved private artifact path | none intended; command enforces read-only method allowlist and no mutation endpoints | no for live until separately approved for this packet-specific route | route implementation/mocked tests only; live final check not run | contract_fix_v1_missing_email_anchor_blocks_consistently; ready requires live lookup; fixture mode and mocked live mode tests; subscriber rows and raw identifiers stay redacted | Packet-specific final safety gate before any exact MailerLite onboarding mutation approval. | Central integration, then one final packet-specific idempotency/suppression check approval. |
