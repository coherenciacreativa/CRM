...
- Source Result Ledger records per-contact source-check outcomes after a batch. Mantis/Codex should distinguish `found_profile_no_requested_bridge`, `not_found_limited_search`, `not_found_exhaustive`, `bridge_found`, and `blocked`, so weak UI searches are not mistaken for exhausted sources.
...
- If Instagram Messages UI returns a plausible candidate, Mantis may and should open the existing thread in read-only mode to verify email/phone/handle/location/context bridges. Reports should include `threadOpenedReadOnly` and the decision reason. Reading an existing thread is allowed; sending, reacting, following/unfollowing, clicking message actions, changing settings, or granting permissions is not.
- Name-only Instagram results are weak/no-write evidence. Mantis must not promote similar names, similar handles, or mutual-follow results as plausible bridges unless the opened thread or another official source exposes exact email, phone, handle, self-identification, strong conversation context, or Alejandro explicitly confirms the handle. For IG/onboarding-origin contacts, prefer searching exact email/phone inside Instagram Messages UI over broad name search.
