...
32. Before daily engagement work, scan saved reports for unprocessed signal packets with `npm run crm:vnext:signal-packet-inbox`.
33. For the standard source-to-score loop, run `npm run crm:vnext:signal-event-pipeline -- --mailerlite-snapshot-file <json> --gmail-reply-discovery-file <json>` or pass Instagram events through `--events-file <json>`.
34. Preview supplied engagement signals with `POST /api/crm-vnext/engagement-signal-preview`.
35. Store useful read-only engagement previews with `npm run crm:vnext:engagement-snapshot-ledger -- --preview-file <json> --write --approved-by <name>`.
36. Read engagement movement history with `GET /api/crm-vnext/engagement-snapshots`.
37. Turn stored movement into a Mantis-ready queue with `GET /api/crm-vnext/engagement-movement-queue`.
38. Prepare a no-send engagement decision brief with `GET /api/crm-vnext/engagement-decision-brief`.
39. Turn that engagement brief into answer-ready questions with `GET /api/crm-vnext/engagement-resolution-loop`; already-enriched contacts are suppressed into internal signal review.
40. Resolve explicitly approved staged merges with `POST /api/crm-vnext/card-merge-review-resolver`.
41. Review stored facts with `GET /api/crm-vnext/identity-review`.
42. Preview exact card changes with `GET /api/crm-vnext/card-rebuild-diff`.
43. Read `GET /api/crm-vnext/community-daily-brief` for community queues plus stored engagement action summaries.
44. Convert that daily picture into an ordered Mantis task list with `GET /api/crm-vnext/daily-operator-handoff`.
45. If needed, read `GET /api/crm-vnext/community-queues`.
46. Inspect a bounded queue with `GET /api/crm-vnext/community-queue-brief?queueId=<queueId>&limit=<n>`.
47. Prepare a no-send decision brief with `GET /api/crm-vnext/community-decision-brief?queueId=<queueId>&limit=<n>`.
48. Inspect one exact person with `GET /api/crm-vnext/person-card?personId=<personId>`.
49. Ask for approval if the next move would touch an external channel.
