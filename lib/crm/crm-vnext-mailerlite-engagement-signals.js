...
const signalForRecord = (record, index, generatedAt, options) => {
  ...
  const activityRows = activityRowsFrom(record);
  const activity = aggregateActivityRows(activityRows, now, options.windowDays);
  const activity90d = aggregateActivityRows(activityRows, now, 90);
  ...
  const opens90d = pickNumber(record, [
    'opens90d',
    'opens_90d',
    'open_count_90d',
    'campaign_opens_90d',
  ]) ?? activity90d.opens;
  const clicks90d = pickNumber(record, [
    'clicks90d',
    'clicks_90d',
    'click_count_90d',
    'campaign_clicks_90d',
  ]) ?? activity90d.clicks;
  const lifetimeOpens = pickNumber(record, [
    'lifetimeOpens',
    'lifetime_opens',
    'aggregate.opens_count',
    'opens_count',
    'total_opens',
  ]) ?? Math.max(opens90d, opens30d);
  const lifetimeClicks = pickNumber(record, [
    'lifetimeClicks',
    'lifetime_clicks',
    'aggregate.clicks_count',
    'clicks_count',
    'total_clicks',
  ]) ?? Math.max(clicks90d, clicks30d);
  const lifetimeSent = pickNumber(record, [
    'lifetimeSent',
    'lifetime_sent',
    'aggregate.sent',
    'sent',
  ...
  const openRate = pickNumber(record, ['openRate', 'open_rate', 'aggregate.open_rate']);
  const clickRate = pickNumber(record, ['clickRate', 'click_rate', 'aggregate.click_rate']);
  ...
  const observedAt = firstIsoDate(record, ['observedAt', 'observed_at', 'snapshotAt', 'snapshot_at', 'generatedAt'])
    ?? options.observedAt
    ?? generatedAt;
  const subscribedAt = firstIsoDate(record, [
    'subscribedAt',
    'subscribed_at',
    'createdAt',
    'created_at',
    'aggregate.subscribedAt',
  ]);
